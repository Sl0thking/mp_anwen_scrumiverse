					...
					<c:forEach items="${tasksOfUserStories[userStory]}" var="task">
						<c:if test="${task.planState.toString() eq 'Done'}">
							<div class="task">
								...
								<a class="glyphicon glyphicon-triangle-right task-link" planstate="${task.planState.toString()}" data-toggle="modal" data-target="#modal-detail"></a>
							</div>
						</c:if>
					</c:forEach>
				</td>
